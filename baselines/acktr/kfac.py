...
class KfacOptimizer():
        ...
        for var in varlist:
            ...
            for stats_var in bpropStats_vars:
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    cov_b = tf.matmul(
                        bpropFactor, bpropFactor, transpose_a=True) / tf.cast(tf.shape(bpropFactor)[0], dtype=tf.float32)
    ...
    def apply_gradients_kfac(self, grads):
        ...
        if self._async:
            ...
            enqueue_op = tf.cond(tf.logical_and(tf.equal(tf.math.floormod(self.stats_step, self._kfac_update), tf.convert_to_tensor(
        ...
        with tf.control_dependencies([global_step_op]):
            ...
            with tf.control_dependencies(dependency_list):
                ...
                if not self._async:
                    ...
                    updateFactorOps = tf.cond(tf.logical_and(tf.equal(tf.math.floormod(self.stats_step, self._kfac_update),
